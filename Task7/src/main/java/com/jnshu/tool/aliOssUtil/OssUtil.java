...
//    OSS ossClient = new OSSClientBuilder().build(ossBean.getEndpoint(),ossBean.getAccessKeyId(),ossBean.getAccessKeySecret());
    ...
    public void upFile(String ObjectName,File file) throws IOException {
        logger.error("阿里上传开始");
        // 创建OSSClient实例。
        OSS ossClient = new OSSClientBuilder().build(ossBean.getEndpoint(),ossBean.getAccessKeyId(),ossBean.getAccessKeySecret());
        ...
        objectMetadata.setContentType(ossBean.getContentype());
        //上传文件，第一参数，存储空间，第二个参数储存在Oss上的位置（文件或对象），第三个参数是传入的文件
        ossClient.putObject(ossBean.getBucketName(),ObjectName, file);
        ossClient.shutdown();
        logger.error("阿里上传完成");
    }
    ...
    public void load(String ObjectName) throws IOException {
        ...
        // 创建OSSClient实例。
        OSS ossClient = new OSSClientBuilder().build(ossBean.getEndpoint(),ossBean.getAccessKeyId(),ossBean.getAccessKeySecret());
        GetObjectRequest request = new GetObjectRequest(ossBean.getBucketName(), ObjectName);
